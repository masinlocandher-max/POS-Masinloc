...
function TrackingView({ tracking, onSend, chat, setChat }: { tracking: Awaited<ReturnType<typeof loadGuestTracking>> | null; onSend: (m: string) => Promise<void>; chat: string; setChat: (m: string) => void }) {
  ...
  return <section className="tracking"><CheckCircle2 /><p>Order #{tracking.order_number}</p><h1>{tracking.status.replaceAll('_', ' ')}</h1><div className="timeline">{labels.map((label, index) => <span key={label} className={index <= (rank[tracking.status] ?? -1) ? 'done' : ''}>{label}</span>)}</div><div className="chat-box"><div className="section-title"><h2>Chat with store</h2><MessageCircle /></div><div className="chat-history">{tracking.messages.length ? tracking.messages.map((m, index) => <p key={`${m.created_at}-${index}`} className={`chat-${m.sender_type}`}><b>{m.sender_type === 'customer' ? 'You' : m.sender_type === 'staff' ? 'Store' : 'Update'}</b>{m.message}</p>) : <span>No messages yet.</span>}</div><div className="chat-compose"><input value={chat} onChange={e => setChat(e.target.value)} maxLength={1000} placeholder="Message the store" /><button disabled={sending || !chat.trim()} onClick={async () => { setSending(true); try { await onSend(chat.trim()) } finally { setSending(false) } }}>Send</button></div></div></section>
}
